...
async fn async_main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        ...
        Commands::Invoke {
            ...
            let contract_spec: Option<sdkt_wasm::ContractSpec> = if let Some(wasm_path) = abi.as_ref()
            {
                let wasm_bytes =
                    fs::read(wasm_path).map_err(|e| format!("Failed to read WASM: {e}"))?;
                Some(parse_contract_spec(&wasm_bytes).map_err(|e| format!("Failed to parse ABI: {e}"))?)
            } else if let Some(id) = abi_contract.as_ref() {
                let inspection = inspect_contract(&client, id).await.map_err(|e| match &e {
